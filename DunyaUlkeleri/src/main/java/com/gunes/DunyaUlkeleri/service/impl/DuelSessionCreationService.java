...
public class DuelSessionCreationService {
    ...
    public CreateDuelSessionResponse createSession(CreateDuelSessionRequest request, boolean quickMatch) {
        ...
        session.setMaxRounds(resolveMaxRounds(category));
    ...
    private int resolveMaxRounds(String category) {
        final String c = safeTrim(category);
        final long count = (c == null || c.isBlank() || "Dünya".equalsIgnoreCase(c))
                ? questionRepository.count()
                : questionRepository.countByContinent(c);
        if (count <= 0) return 5;
