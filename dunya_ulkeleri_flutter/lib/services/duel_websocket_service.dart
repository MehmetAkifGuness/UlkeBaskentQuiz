// lib/services/duel_websocket_service.dart
import 'dart:async';
import 'dart:convert';
import 'dart:math';
...
class DuelWebSocketService {
  ...
  void Function(DuelSessionState state)? _onState;
  ...
  String _resolveWsUrl() {
    ...
    if (apiBase.isEmpty) return '/ws/duel';
  ...
  void connect({
    ...
    required void Function(DuelSessionState state) onState,
      ...
      config: StompConfig.sockJS(
        ...
        onConnect: (frame) {
          ...
          _stateUnsubscribe = _client?.subscribe(
            destination: '/topic/duel/$sessionId',
            callback: (frame) {
              if (generation != _generation) return;
              try {
                ...
                if (decoded is Map<String, dynamic>) {
                  onState(DuelSessionState.fromJson(decoded));
                } else if (decoded is Map) {
                  onState(
                    DuelSessionState.fromJson(
          ...
          _errorUnsubscribe = _client?.subscribe(
            destination: '/topic/duel/$sessionId/errors',
            callback: (frame) {
              if (generation != _generation) return;
              if (frame.body == null) return;
              try {
  ...
  void submitAnswer(SubmitDuelAnswerRequest request) {
    _client?.send(
      destination: '/app/duel.answer',
      body: jsonEncode(request.toJson()),
    );
  }

  void requestState(String sessionId) {
    _client?.send(
      destination: '/app/duel.state',
      body: jsonEncode({'sessionId': sessionId}),
    );
  }

  void leaveSession(LeaveDuelSessionRequest request) {
    _client?.send(
